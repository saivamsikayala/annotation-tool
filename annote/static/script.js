...
function responseListener() {
    ...
    import_annotations_from_json( d );
    return;
    }
...
function _dmiat_deposit_metadata() {
    setTimeout(function() {
        ...
        var img_metadata = pack_via_metadata('json');
        var img_metadata_str = img_metadata[0];
        var timenow = new Date().toUTCString();
        ...
        payload['content'] = img_metadata_str;
        ...
        gh.open('PATCH', url);
        gh.setRequestHeader("Content-type", "application/json");
...
function submit_annotation(type){
    var all_region_data = pack_via_metadata(type);
    alert(all_region_data);
    var all_region_data_blob = new Blob(all_region_data, {type: 'text/json;charset=utf-8'});
    var saveData = $.ajax({
      type: 'POST',
      url: "image/annotate",
      //data: JSON.stringify(all_region_data), 
      data: all_region_data[0], 
